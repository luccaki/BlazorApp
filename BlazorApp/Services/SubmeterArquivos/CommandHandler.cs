...
namespace BlazorApp.Services
{
    public partial class SubmeterArquivos
    {
        public class CommandHandler
        {
            ...
            {
                Console.WriteLine($"entrou {command.Horas}");
                if (command.Arquivo is null)
                {
                    Console.WriteLine("Sem Arquivo");
                    return;
                }
                Console.WriteLine($"{command.Arquivo.Name}");

                StreamReader file = new StreamReader(command.Arquivo.OpenReadStream());
                StringBuilder saida = new StringBuilder();

                String line = await file.ReadLineAsync();
                for (int i = 0; line != null; i++, line = await file.ReadLineAsync())
                {
                    if (line.Contains("-->"))
                    {
                ...
                Db.Add(new Arquivos(fileBytes, command.Arquivo.Name));
                Db.SaveChanges();
